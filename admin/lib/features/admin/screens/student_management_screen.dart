...
class _StudentManagementScreenState extends State<StudentManagementScreen> {
  ...
  Widget _studentCard(Map<String, dynamic> student, int index) {
    ...
    final category = student['category'] ?? "N/A";
    final avatar =
        student['applicantPhoto'] ??
        "https://ui-avatars.com/api/?name=$name&background=random";
    final score = "${student['percentageCGPA']}%";

    final email = student['email'] ?? "N/A";
    final phone = student['mobile'] ?? "N/A";
    final gender = student['gender'] ?? "N/A";
    final session = student['sessionYear'] ?? "N/A";
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Text(
                          student['admissionNumber'] ?? student['studentId'] ?? "#STU${index + 1}",
                          style: const TextStyle(
                            color: Colors.white,
                            fontWeight: FontWeight.w900,
                        ...
                        child: Column(
                          children: [
                            ...
                            _detailRow(
                              ...
                              "Born: ${student['dob'] ?? 'N/A'}",
                            ),
                            const SizedBox(height: 10),
                            _detailRow(
                              Icons.history_edu_rounded,
                              "Session: $session | $gender",
                            ),
                          ],
                        ),
                      ),
                    ...
                    children: [
                      Expanded(
                        child: _actionBtn(
                          ...
                          () => Navigator.push(
                            context,
                            _slideRoute(StudentDetailScreen(student: student)),
                          ),
                        ),
                      ),
                      const SizedBox(width: 8),
                      _miniActionBtn(
                        ...
                        () async {
                          final result = await Navigator.push(
                            context,
                            _slideRoute(CreateStudentScreen(student: student)),
                          );
                          if (result == true) {
                            _loadData();
                          }
                      ...
                      if (status == 'Pending') ...[
                        _miniActionBtn(
                          Icons.check_rounded,
                          Colors.green,
                          () => _handleUpdateStatus(student['_id'], 'Accepted'),
                        ),
                        const SizedBox(width: 8),
                        _miniActionBtn(
                          Icons.close_rounded,
                          Colors.red,
                          () => _handleUpdateStatus(student['_id'], 'Rejected'),
                        ),
                        const SizedBox(width: 8),
                      ],
                      _miniActionBtn(
