...
class _CreateStudentScreenState extends State<CreateStudentScreen> {
  ...
  Future<void> _loadApplications() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _acceptedApplications = apps.where((a) => a['status'] == 'Accepted').toList();
        });
      }
    } catch (e) {
  ...
  void _autofillFromApplication(Map<String, dynamic> app) {
    setState(() {
      _selectedApplicationId = app['_id'];
      _firstNameCtrl.text = app['firstName'] ?? '';
      _lastNameCtrl.text = app['lastName'] ?? '';
      _dobCtrl.text = app['dob'] ?? '';
      _emailCtrl.text = app['email'] ?? '';
      ...
      if (_sessions.contains(app['sessionYear'])) _selectedSession = app['sessionYear'];
      if (_categories.contains(app['category'])) _selectedCategory = app['category'];
      if (_genders.contains(app['gender'])) _selectedGender = app['gender'];
      if (_qualifications.contains(app['highestQualification'])) _selectedQualification = app['highestQualification'];
  ...
  Future<void> _selectDate(BuildContext context) async {
    ...
    if (picked != null) {
      setState(() {
        ...
        // Default password to DOB if password is empty
  ...
  Widget _stepPersonal({required double width, required Key key}) {
    ...
    return Column(
      ...
      children: [
        ...
        // --- ADDED DROPDOWN FOR APPLICATIONS ---
        if (_acceptedApplications.isNotEmpty && widget.student == null) ...[
          Container(
            ...
            child: Column(
              ...
              children: [
                const Text(
                  "Import from Accepted Applications (Optional)",
                  style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: AppColors.primaryRed),
                ),
                const SizedBox(height: 12),
                Container(
                    ...
                    child: DropdownButton<String>(
                      ...
                      items: [
                        const DropdownMenuItem<String>(
                          value: null,
                          child: Text("Add new student (Manual Entry)"),
                        ),
                        ..._acceptedApplications.map((app) {
                          return DropdownMenuItem<String>(
                            value: app['_id'],
                            child: Text("${app['firstName']} ${app['lastName']} - ${app['selectedProgram']}"),
                          );
                        })
                      ],
                      onChanged: (id) {
                        setState(() => _selectedApplicationId = id);
                        if (id != null) {
                          final selectedApp = _acceptedApplications.firstWhere((a) => a['_id'] == id);
                          _autofillFromApplication(selectedApp);
                        } else {
                          // Allow manual creation empty fields
                          _existingProfileUrl = null;
                        }
                      },
                    ),
  ...
  Future<void> _saveStudent() async {
    ...
    try {
      ...
      final studentData = {
        if (_selectedApplicationId != null) "applicationId": _selectedApplicationId,
      ...
      // 4. Submit to Backend
      if (widget.student != null) {
        await StudentService.updateStudent(widget.student!['_id'], studentData);
      } else {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(widget.student != null ? "Student updated successfully!" : "Student saved successfully!"),
